...
class Performance():
    ...
    def mesure(self, process_name, start_mark=None, end_mark=None):
        '''計測結果作成
        指定された開始マークから終了マークまでの処理時間を計測し結果を保持する
        Args:
            process_name : 計測結果名称
            start_mark : 計測開始マーク。未指定の場合、本クラスのインスタンスが生成されたタイミングを利用する
            end_mark : 計測終了マーク。未制定の場合、本メソッドが呼ばれたタイミングを利用する。
        '''
        start_time = self.times.get(start_mark, self.initial_time) if start_mark is not None else self.initial_time
        end_time = self.times.get(end_mark, time.time_ns()) if end_mark is not None else time.time_ns()
    ...
    def now(self):
        '''現在の時刻を返却する
        Returns:
            int : epochからのnano秒を返却します（Unixでは1970/1/1 0:0:0）
        '''
        return time.time_ns()
    ...
    def get_simple_entries(self):
        ...
        '''
        return {process_name : result['duration']/(1000 * 1000) for process_name, result in self.marks.items()} 
